...
exports.checkInTicket = async (req, res) => {
  ...
  const completedBooking = ticketBooking || result.booking;
  await recordAuditLog(req, {
    action: "ticket.check_in",
    resourceType: "Booking",
    resourceId: result.booking.id,
    metadata: {
      ticketCode: completedBooking.ticket_code,
      alreadyCheckedIn: result.alreadyCheckedIn,
      checkedInAt: completedBooking.checked_in_at,
      movie: completedBooking.show?.movie?.title || null,
      theater: completedBooking.show?.screen?.theater?.name || null,
      screen: completedBooking.show?.screen?.name || null,
      seats: (completedBooking.bookingSeats || []).map(seatLabel).filter(Boolean),
    },
  });
  res.json(
    buildTicketResponse(completedBooking, {
...
exports.getMyCheckIns = async (req, res) => {
  const { page = 1, limit = 20 } = res.locals.validated.query;
  const qb = AppDataSource.getRepository("AuditLog")
    .createQueryBuilder("audit")
    .where("audit.actor_user_id = :actorUserId", { actorUserId: req.user.id })
    .andWhere("audit.action = :action", { action: "ticket.check_in" });
  const [items, total] = await qb.orderBy("audit.created_at", "DESC").skip((page - 1) * limit).take(limit).getManyAndCount();
  res.json({
    data: items.map((item) => {
      let metadata;
      try { metadata = item.metadata_json ? JSON.parse(item.metadata_json) : {}; } catch (_error) { metadata = {}; }
      return { id: item.id, bookingId: item.resource_id, createdAt: item.created_at, ...metadata };
    }),
