...
exports.googleLogin = async (req, res) => {
  const profile = await verifyGoogleIdToken(res.locals.validated.body.credential);
  const repository = getUserRepository();
  const user = await findOrCreateGoogleUser(profile, repository, userRelations);

  res.json({ message: "Google login successful", ...(await issueSession(user, res)) });
};
