...
const { issueSession } = require("../services/authTokenService");
const { verifyPayOSReturnSession } = require("../payments/payosReturnSession");
...
exports.reconcilePayOSReturn = async (req, res) => {
  ...
  if (req.query.session) {
    verifyPayOSReturnSession(String(req.query.session), {
      orderCode,
      userId: result.booking.user?.id,
    });
    const user = await AppDataSource.getRepository("User").findOne({
      where: { id: result.booking.user.id },
      relations: { theaterAssignments: { theater: true } },
    });
    if (!user || user.is_active === false) {
      throw new AppError(403, "ACCOUNT_LOCKED", "Account is unavailable");
    }
    session = await issueSession(user, res);
  }
