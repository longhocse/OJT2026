...
// SQL Server stores these timestamps as UTC without a timezone suffix. Dashboard
// dates are business dates in Vietnam, so local midnight is 17:00 UTC of the
// previous day. Keeping the conversion here also makes preset filters and chart
// grouping use the same boundary.
const BUSINESS_TIMEZONE_OFFSET_MINUTES = 7 * 60;
const parseDateStart = (date) => {
  const start = new Date(`${date}T00:00:00.000Z`);
  start.setUTCMinutes(start.getUTCMinutes() - BUSINESS_TIMEZONE_OFFSET_MINUTES);
  return start;
};
...
exports.getDashboardStats = async (req, res) => {
  const query = res.locals.validated.query;
  const applyCinemaFilter = (qb, alias) => {
    if (query.cinemaId) qb.andWhere(`${alias}.id = :dashboardCinemaId`, { dashboardCinemaId: query.cinemaId });
    return qb;
  };
  ...
  applyCinemaFilter(summaryQb, "summaryTheater");
  applyDateRange(summaryQb, query, "booking.created_at");
  const summary = await summaryQb.getRawOne();

  const localBookingDate = `DATEADD(MINUTE, ${BUSINESS_TIMEZONE_OFFSET_MINUTES}, booking.created_at)`;
  const localBookingDateKey = `CONVERT(varchar(10), ${localBookingDate}, 23)`;
  const seriesQb = bookingRepo
    ...
    .select(localBookingDateKey, "date")
    ...
    .addSelect("SUM(booking.refunded_amount)", "refund")
    // SQL Server requires the selected non-aggregate expression to match the
    // GROUP BY expression exactly.
    .groupBy(localBookingDateKey)
    .orderBy(localBookingDateKey, "ASC");
  applyTheaterScope(seriesQb, req, "seriesTheater");
  applyCinemaFilter(seriesQb, "seriesTheater");
  ...
  applyCinemaFilter(bookedSeatQb, "seatTheater");
  ...
  applyCinemaFilter(capacityQb, "capacityTheater");
