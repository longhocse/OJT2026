...
const { signPayOSReturnSession } = require("./payosReturnSession");
...
class PayOSPaymentProvider extends PaymentProvider {
  async createIntent({ paymentId, amount, booking }) {
    ...
    const returnUrl = new URL(absoluteUrl(env.PAYOS_RETURN_URL, "/payment/payos/return"));
    returnUrl.searchParams.set(
      "session",
      signPayOSReturnSession({ orderCode, userId: booking?.user?.id }),
    );
    const body = {
      orderCode,
