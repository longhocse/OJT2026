...
test("login, Google login and verifyEmail normalize auth responses; register returns verification status", async () => {
  api.post
  ...
  await expect(authService.googleLogin("google-id-token")).resolves.toMatchObject({
    token: "token",
    user: { id: "user-1" },
  });
  await expect(
