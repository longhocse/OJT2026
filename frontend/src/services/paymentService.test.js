...
test("payment service normalizes payment, refund, cash, ticket and check-in responses", async () => {
  api.get
    ...
    .mockResolvedValueOnce({ data: { paid: true, payosStatus: "PAID" } })
  ...
  await expect(paymentService.reconcilePayOS("123456")).resolves.toEqual({
    paid: true,
    payosStatus: "PAID",
  });
  ...
  expect(api.get).toHaveBeenCalledWith("/payments/payos/reconcile", {
    params: { orderCode: "123456" },
  });
});
