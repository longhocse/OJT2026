...
describe("Axios authentication behavior", () => {
  ...
  test("keeps the session when refresh cannot reach the backend", async () => {
    jest.spyOn(axios, "post").mockRejectedValueOnce(new Error("network unavailable"));
    ...
    expect(error.sessionVerificationDeferred).toBe(true);
