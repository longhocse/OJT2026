...
export default function PayOSReturnPage() {
  ...
  const isCashierSale = booking?.cashierSale === true;
  ...
  const reconcileQuery = useQuery({
    ...
    queryFn: () => paymentService.reconcilePayOS(orderCode, returnSession),
  ...
  const reconciledPaid = reconcileQuery.data?.paid === true;
  const status =
    bookingQuery.data?.status ||
    reconcileQuery.data?.bookingStatus ||
    booking?.status ||
    "pending_payment";
  const paymentStatus =
    bookingQuery.data?.payment_status ||
    reconcileQuery.data?.paymentStatus ||
    booking?.payment?.status ||
    "pending";
  const confirmed = reconciledPaid || status === "confirmed" || paymentStatus === "paid";

  React.useEffect(() => {
    if (reconcileQuery.data?.session?.token && reconcileQuery.data?.session?.user) {
      dispatch(setCredentials(reconcileQuery.data.session));
    }
  }, [dispatch, reconcileQuery.data?.session]);
    ...
    <main className="flex min-h-screen items-center justify-center px-4 pb-16 pt-24">
      <section className="w-full max-w-2xl rounded-2xl bg-surface-container p-8 text-center">
          ...
          {isCancel
            ...
            : confirmed
              ...
              : reconcileQuery.isError
                ? "Chưa thể đối soát với PayOS. Hệ thống sẽ tiếp tục nhận xác nhận tự động qua webhook."
                : "PayOS đã chuyển bạn về MovieTap. Hệ thống đang đối soát giao dịch, có thể mất vài giây."}
        </p>
        ...
        <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
          <button
            type="button"
            onClick={() => leave(isCashierSale ? "/cashier/history" : "/my-bookings")}
          ...
          <button
            type="button"
            onClick={() => leave(isCashierSale ? "/cashier" : "/movies")}
            className="rounded-lg border border-white/20 px-6 py-3 font-semibold"
          >
            {isCashierSale ? "Bán vé tiếp" : "Tiếp tục xem phim"}
          </button>
        </div>
      </section>
    </main>
