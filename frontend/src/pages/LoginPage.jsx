...
import { roleHomePath, roleOwnsPath } from "../utils/roles";
...
const AuthForm = ({ mode }) => {
  ...
  const submit = async (values) => {
    ...
    try {
      ...
      navigate(typeof requested === "string" && roleOwnsPath(response.user.role, requested) ? requested : roleHomePath(response.user.role), { replace: true });
    } catch (error) {
      setFormError(
  ...
  const destinationFor = (response) => {
    const requested = location.state?.from;
    return typeof requested === "string" && roleOwnsPath(response.user.role, requested)
      ? requested
      : roleHomePath(response.user.role);
  };
  ...
  const submitGoogle = async (credential) => {
    ...
    try {
      ...
      navigate(destinationFor(response), { replace: true });
    } catch (error) {
      setFormError(error.response?.data?.message || "Không thể đăng nhập bằng Google.");
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit(submit)} className="space-y-4" noValidate>
      ...
      <GoogleSignInButton onCredential={submitGoogle} disabled={isSubmitting} />
    </form>
  );
};
