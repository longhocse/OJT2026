...
export default function AdminGenres() {
  const canManageCatalog = useSelector((state) => state.auth.user?.role === "admin");
              ...
              <table className="w-full text-left text-sm text-slate-300">
                <thead className="bg-slate-800/50 text-slate-400 text-xs uppercase tracking-wider border-b border-white/5">
                  <tr>
                    ...
                    {canManageCatalog && <th className="px-6 py-4 font-semibold text-center w-1/3">Thao tác</th>}
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {(query.data || []).map((genre) => (
                    <tr key={genre.id} className="group hover:bg-white/5 transition-colors">
                      {canManageCatalog && <td className="px-6 py-4 text-center">
                  ...
                  {(query.data || []).length === 0 && (
                    <tr>
                      <td colSpan={canManageCatalog ? 3 : 2} className="px-6 py-10 text-center text-slate-500">
