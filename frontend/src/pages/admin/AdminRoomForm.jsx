...
const AdminRoomForm = () => {
  ...
  const assignedCinemaIds = useMemo(
    () => new Set((user?.theaterAssignments || []).filter((item) => item.is_active && item.theater?.id).map((item) => String(item.theater.id))),
    [user?.theaterAssignments],
  );
  const availableCinemas = useMemo(
    () => user?.role === "manager" ? (cinemasQuery.data || []).filter((cinema) => assignedCinemaIds.has(String(cinema.id))) : (cinemasQuery.data || []),
    [assignedCinemaIds, cinemasQuery.data, user?.role],
  );
  ...
  useEffect(() => {
    if (id || user?.role !== "manager" || availableCinemas.length !== 1) return;
    setValue("theater.id", availableCinemas[0].id, { shouldValidate: true });
  }, [availableCinemas, id, setValue, user?.role]);
  ...
  return (
    <main className="mx-auto max-w-4xl p-6">
      <div className="mb-6 flex items-center gap-4">
        <button type="button" aria-label="Quay lại" onClick={() => navigate(`${basePath}/rooms`)}>
