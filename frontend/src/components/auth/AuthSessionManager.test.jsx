...
test("purges a persisted session only when the backend rejects it", async () => {
...
test("keeps a persisted session during a temporary network failure", async () => {
  authService.getMe.mockRejectedValue(new Error("network unavailable"));
  const store = configureStore({
    reducer: { auth: authReducer },
    preloadedState: {
      auth: { user: null, token: "persisted", isAuthenticated: false, verificationStatus: "idle" },
    },
  });
  render(
    <Provider store={store}>
  ...
  await waitFor(() => expect(authService.getMe).toHaveBeenCalledTimes(2));
  expect(clearClientSession).not.toHaveBeenCalled();
});
