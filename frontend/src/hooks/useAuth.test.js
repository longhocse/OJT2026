...
describe("useAuth", () => {
  ...
  test("stores a successful Google login response in Redux", async () => {
    const response = {
      token: "google-token",
      user: { id: "user-google", role: "customer" },
    };
    authService.googleLogin.mockResolvedValue(response);
    const { result, store } = renderAuthHook();

    await act(async () => result.current.googleLogin("google-id-token"));

    expect(authService.googleLogin).toHaveBeenCalledWith("google-id-token");
    expect(store.getState().auth.token).toBe("google-token");
  });
